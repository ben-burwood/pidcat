...
class FakeStdinProcess:
    """ducktype of the subprocess.Popen object"""
...
def main():
    ...
    named_processes = map(
        lambda package: package if package.find(":") != len(package) - 1 else package[:-1],
        named_processes,
    )
    ...
    RULES = {
        ...
        re.compile(r"^(StrictMode policy violation)(; ~duration=)(\d+ ms)"): r"%s\1%s\2%s\3%s" % (termcolor(RED), RESET, termcolor(YELLOW), RESET),
    }
